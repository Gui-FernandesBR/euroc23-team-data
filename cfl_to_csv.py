    ...
    for file in files:
        ...
        for name, df in dictionary.items():
            ...
            if "q0_estimated" in df.columns:
                df["q0_estimated"] = round(df["q0_estimated"] / 10, 4)
                df["q1_estimated"] = round(df["q1_estimated"] / 10, 4)
                df["q2_estimated"] = round(df["q2_estimated"] / 10, 4)
                df["q3_estimated"] = round(df["q3_estimated"] / 10, 4)
            if "ts" in df.columns:
