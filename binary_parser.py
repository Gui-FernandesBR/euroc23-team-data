"""This is a simple copy from this file: 
https://github.com/catsystems/cats-logs/blob/a82986e2768a6b31f0d51a26b295518e3b6ec831/log_parsing/binary_parser.py

Ideally, we should install this file as a package and import it here, but the
code is not organized as a package yet so the easiest way was to copy and paste.

There won't be any maintenance on this file.

**The only modification is that the print() messages were commented.
"""
...
from embedded_constants import ERROR_MAP, EVENT_MAP, REC_TYPE
...
def read_log(
    log_path: str, plot_output_dir: str, raw_output_dir: str, processed_output_dir: str
) -> str:
...
def report_non_periodic_recordings(
    log_dict: Dict[str, list], state_map: Dict[int, str]
) -> int:
    ev_liftoff_ts = 0
    # print("\nEvents:")
    ...
    return (
        {
            "imu_df": imu_df,
            "baro_df": baro_df,
            "flight_info_df": flight_info_df,
            "orientation_info_df": orientation_info_df,
            "filtered_data_info_df": filtered_data_info_df,
...
def parse_log(log_b: bytes):
    ...
    try:
        while i < len(log_b):
            ...
            if t_without_id == REC_TYPE.IMU:
                ...
                pressure, temperature = struct.unpack("<LL", log_b[i : i + 8])
                ...
                est_0, est_1, est_2, est_3 = struct.unpack("<hhhh", log_b[i : i + 8])
                ...
                event_info.append({"ts": ts, "event": event, "out_idx": out_idx})
                ...
                error_info.append({"ts": ts, "error": error})
                i += 4
                ...
                voltage_info.append({"ts": ts, "voltage": voltage})
                i += 2
            else:
                # print(t)
                # print(f"ERROR at {i}")
    ...
    return {
        ...
        "filtered_data_info": filtered_data_info,
